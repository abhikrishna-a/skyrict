...
- **Live Fabric trial execution**: the paste-able T-SQL load script
  (`fabric/samples/warehouse-load.sql`, generated by `gold-tsql`) and the
  parity suite are ready for a Fabric Warehouse SQL endpoint. Running them
  against a real 60-day trial workspace is a deployment step outside this
  repo (see `docs/runbooks/sky-118-free-setup.md` §5a).
- **BI-PBI-001 measure validation** against the live Warehouse is downstream
  of that deployment (already verified against Neon + Power BI Desktop).
