...
export function DatePicker({
    ...
    useEffect(() => {
        ...
        function handlePointerDown(event: PointerEvent) {
            ...
            if (target.closest('[role="dialog"][aria-label="Choose date"]'))
                return;
    ...
    const cells = useMemo<Cell[]>(() => {
        ...
        for (let i = lead; i > 0; i--)
            pushCell(new Date(view.y, view.m - 1, 1 - i), false);
        for (let d = 1; d <= daysInMonth; d++)
            pushCell(new Date(view.y, view.m - 1, d), true);
        // Trailing days to complete the final week (minimum five rows).
        let nextOffset = 1;
        while (out.length < 35 || out.length % 7 !== 0) {
            pushCell(
                new Date(view.y, view.m - 1, daysInMonth + nextOffset),
                false,
            );
            nextOffset++;
        }
    ...
    const moveHighlight = useCallback(
        (days: number) => {
            ...
            if (next.m !== view.m || next.y !== view.y)
                setView(clampView({ y: next.y, m: next.m }, lockYear));
        },
    ...
    function shiftView(months: number) {
        setView((current) => {
            ...
            return clampView(
                { y: shifted.getFullYear(), m: shifted.getMonth() + 1 },
                lockYear,
            );
        });
    }
    ...
    const atStart = view.y === minYear && view.m === 1;
            ...
            <div className="relative">
                ...
                >
                    <span className="truncate">
                        {opts.items.find((i) => i.key === opts.value)?.label ??
                            opts.value}
                    </span>
