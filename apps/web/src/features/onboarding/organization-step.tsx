...
function OrganizationStep({
    ...
    vt: string;
    plan?: string;
    /** Billing interval chosen on the Plan step; defaults to `month`. */
    interval?: string;
    /** Currency chosen on the Plan step; defaults to `usd`. */
    currency?: string;
}) {
    ...
    function handleCompleteProvisioning() {
        if (!createdSlug || !createdTenantId) return;
        const selectedPlan =
            plan &&
            ["starter", "professional", "business", "enterprise"].includes(plan)
                ? plan
                : "starter";
        const selectedInterval = interval === "year" ? "year" : "month";
        const selectedCurrency =
            currency === "inr" || currency === "gbp" || currency === "eur"
                ? currency
                : "usd";
        // Persist the full wizard context so the Billing/Review steps survive the
        // Stripe redirect (its return URL only carries plan+interval+checkout).
        saveWizardSession({
            email,
            vt,
            tenantId: createdTenantId,
            slug: createdSlug,
            plan: selectedPlan as BillingPlanId,
    ...
    async function onSubmit(values: OrganizationValues) {
        ...
        try {
            slugAvailable = (
                await checkWorkspaceSlug({ slug: values.workspaceSlug })
            ).available;
        } catch (err) {
            setError("root", {
                type: "manual",
        ...
        try {
            const result = await createOrganization({
                ...
                // The plan was chosen before this step; omit when absent so the
                // backend defaults to `starter`.
                planId: plan,
    ...
    return (
        ...
        >
                        ...
                        <SelectTrigger id="phoneCountry" className="h-10">
                            <span className="flex items-center gap-1.5">
                                {selectedPhoneCountry ? (
                                    <span
                                        className={`fi fi-${selectedPhoneCountry.code.toLowerCase()}`}
                                    />
                                ) : null}
                                <span>
                                    {selectedPhoneCountry
                ...
                >
                    <SelectTrigger id="addressCountry" className="h-10">
                        <span className="flex items-center gap-1.5">
                            {addressCountryValue ? (
                                <span
                                    className={`fi fi-${addressCountryValue.toLowerCase()}`}
                                />
                            ) : null}
                            <span>
                                {addressCountries.find(
                                    (c) => c.code === addressCountryValue,
            ...
            >
                I&apos;m authorized to set up this organization and make billing
...
function CheckboxRow({
    ...
    return (
        <div>
            ...
            {policyLink ? (
                <div className="mt-0.5 pl-6">{policyLink}</div>
            ) : null}
            {error ? (
                <p
                    id={`${id}-error`}
